	...
	keyNum_t *key, qboolean down )
{
	...
	*key = 0;
	...
	#if 0 //ioq3-urt: Disable it for now since it interferes severely with ALTGR.
...
static void IN_ProcessEvents( void )
{
	...
	#ifdef WIN32
		if (			clu.raw.in_rawmouse->integer && clu.raw.registered)  
			clu.raw.IN_RawProcess();
		else if (	clu.raw.in_rawmouse->integer && !clu.raw.registered)
			clu.raw.IN_RawInit();
		else if (	clu.raw.registered)
			clu.raw.IN_RawDeregister();
	#endif
	...
	while( SDL_PollEvent( &e ) )
	{
		switch( e.type )
		{
			...
			case SDL_MOUSEMOTION:
			...
			#ifdef WIN32 
				if (!clu.raw.registered) {
			#endif
					if (clu.mouse.cl_drawmouse->integer) { 
						clu.mouse.count++;
						clu.mouse.sumx += abs(e.motion.xrel);
						clu.mouse.sumy += abs(e.motion.yrel);
						if(clu.mouse.maxx < abs(e.motion.xrel)) clu.mouse.maxx = abs(e.motion.xrel);
						if(clu.mouse.maxy < abs(e.motion.yrel)) clu.mouse.maxy = abs(e.motion.yrel);
					}
			#ifdef WIN32
				}
			#endif
				...
				#ifdef WIN32
					//ioq3-urt: these do get sent even with RIDEV_NOLEGACY flag.
					if (!clu.raw.in_rawmouse->integer) 
				#endif
						if( mouseActive)
			...
			case SDL_MOUSEBUTTONUP:
				#ifdef WIN32
					//ioq3-urt: we could have used RIDEV_NOLEGACY but possible SDL issue creates cursor quirks in window mode.
					if (!clu.raw.in_rawmouse->integer) {
				#endif
						{
							unsigned char b;
