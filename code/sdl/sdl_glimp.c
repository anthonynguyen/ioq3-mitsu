...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder)
{
	...
	for (i = 0; i < 16; i++)
	{
		...
		SDL_WM_SetCaption(CLIENT_WINDOW_TITLE, CLIENT_ICONIFIED_TITLE);
		SDL_ShowCursor(0);
		
		#ifdef WIN32
			clu.raw.init_WndProc();
		#endif
...
void GLimp_Init( void )
{
	...
	if (!(!r_noborder->integer && !strcmp(clu.r_windowPosition->string, "0,0"))) //let's not mess it up for border windows on default value
		Sys_SetEnv( "SDL_VIDEO_WINDOW_POS", clu.r_windowPosition->string);
