...
//	Com_Printf("playing %s\n", sfx->soundName);
	...
	allowed = clu.s_smpRepeat->integer; //ioq3-urt: let's be much more forgiving about these two
	if (entityNum == listener_number) {
		allowed = clu.s_smpRepeat->integer * 2 ; //ioq3-urt: on default (8) it breaks automatic rifle sounds on timescale 1!
	}
...
//		s_numSfx = 0;
	...
	#ifndef NO_DMAHD
		if (dmaHD_Enabled()) {
			Com_Printf("* dmaHD Audio Enabled\n");
			return dmaHD_Init(si);
		}
	#endif
	
	Com_Printf("* Base Audio Enabled\n");
	return qtrue;
}
