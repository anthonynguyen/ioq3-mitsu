...
qboolean S_AL_Init( soundInterface_t *si )
{
	...
	if( !QAL_Init( s_alDriver->string ) )
	{
		...
		#ifdef USE_VOIP
			if (cl_voip->integer) Com_Printf(S_COLOR_YELLOW "VoIP is enabled but a required OpenAL library can not be found.\n");
		#endif
		return qfalse;
	}
	...
	else
	{
		...
		else
		{
			...
			if(alCaptureDevice && cl_voip->integer) Com_Printf("* VoIP Initialized\n"); //weird place to be but it's proper for this purpose.
			else if (!alCaptureDevice && cl_voip->integer)  Com_Printf(S_COLOR_YELLOW "An OpenAL capture device is not found; it is required by VoIP.\n"); 
		}
	}
#endif
